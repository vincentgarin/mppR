...
#' training set (p.ts = R2.ts/heritability). Two types of p.ts are calculated
#' from adjusted and unadjusted R squared. All R global and partial R squared
#' are adjusted using formula 2 from Utz et al. (2000). 
#' 
#' For each single QTL effect difference and single R squared are also
#' calculated. These partial R squared are also returned raw and adjusted.
#' For details about R squared computation in the training set look at
#' \code{\link{QTL_R2}}.}
#' 
...
#' R2.vs by the heritability. 
#' 
#' Partial QTL predicted adjusted and unadjusted R squared are also
#' calculated using difference between a full model and a model minus the ith
#' QTL (difference R squared) or single QTL position model (single R squared).
#' For details about the predicted R squared see \code{\link{QTL_pred_R2}}.}
#' 
#' }
#' 
...
#' 
#' \item{Average  proportion of explained genetic variance in the training set
#' difference adjusted R squared (av.pts.adj.d).}
#' \item{Average  proportion of predicted genetic variance in the validation set
#' difference adjusted R squared (av.pvs.adj.d).}
#' \item{Relative bias explained genetic variance difference adjusted R squared
#' (bias.adj.d).}
#' 
#' \item{Average  proportion of explained genetic variance in the training set
...
#' 
#' \item{Average  proportion of explained genetic variance in the training set
#' single QTL adjusted R squared (av.pts.adj.s).}
#' \item{Average  proportion of predicted genetic variance in the validation set
#' single QTL adjusted R squared (av.pvs.adj.s).}
#' \item{Relative bias explained genetic variance single QTL adjusted R squared
#' (bias.adj.s).}
#'
#' }
#' 
...
#' 
#' \item{p.vs.adj}{Table with the proportion of predicted genetic variance from
#' adjusted R squared in the validation set for each (Rep*k) repetition.}
#' 
#' \item{bias.adj}{Explained genetic variance bias between training and validation
#' set from adjusted R squared 1-(p.vs/p.ts).}
#' 
...
mpp_CV <- function(pop.name = "MPP_CV", trait.name = "trait1",
                   mppData, her = NULL, Rep = 10, k = 5, Q.eff = "cr",
                   ...
                   within.cross = TRUE, parallel = FALSE, cluster = NULL,
                   silence.print = FALSE, output.loc = getwd()) {
  
  # 1. Check the validity of the parameters that have been introduced
  ###################################################################

  check.mpp.cv(mppData = mppData, her = her, Q.eff = Q.eff, VCOV = VCOV,
  ...
  N.QTL <- p.ts <- p.vs <- bias <- matrix(0, k, Rep)

  p.ts.adj <- p.vs.adj <- bias.adj <- matrix(0, k, Rep)

  QTL.positions <- rep(0, dim(mppData$map)[1])
  N.Qeff.est.ts <- rep(0, dim(mppData$map)[1])
  N.Qeff.est.vs <- rep(0, dim(mppData$map)[1])

  QTL.pts.s <- QTL.pvs.s <- QTL.pts.adj.s <- QTL.pvs.adj.s <-
    rep(0, dim(mppData$map)[1])

  QTL.pts.d <- QTL.pvs.d <- QTL.pts.adj.d <- QTL.pvs.adj.d <-
    rep(0, dim(mppData$map)[1])
      ...
      if (!is.null(cofactors)) {
        ...
        ### 4.3 compute the CV statistics (N.QTL, p.ts. etc.)
          ...
          R2.ts <- QTL_R2(mppData = mppData.ts, QTL = QTL, Q.eff = Q.eff,
                          par.clu = par.clu, VCOV = VCOV, LR.R2 = LR.R2)

          # try to calcuate R2 with the linear method if it fail with LR R2
          # for VCOV = "cr.err".

          if(((VCOV == "cr.err") & (LR.R2)) & is.na(R2.ts[[1]][1])){

            R2.ts <- QTL_R2(mppData = mppData.ts, QTL = QTL, Q.eff = Q.eff,
                         par.clu = par.clu, VCOV = "h.err")
            
        message <- paste("In CV repetition", i, "fold ", j, "the computation of",
          ...
          # test if the R2 computation failed change NA for 0
          ...
          p.vs.adj[j, i] <- R2.vs$glb.adj.R2/her
          bias.adj[j, i] <- round(1 - (p.vs.adj[j, i]/p.ts.adj[j, i]), 2)
          ...
          QTL.pts.s <- update.res(input = R2.ts$part.R2.sg/her, output = QTL.pts.s,
                                  Q.names = QTL.names, mk.list = mk.list)
          
          QTL.pts.adj.s <- update.res(input = R2.ts$part.adj.R2.sg/her,
                                      output = QTL.pts.adj.s,
                                  Q.names = QTL.names, mk.list = mk.list)
          
          QTL.pvs.s <- update.res(input = R2.vs$part.R2.sg/her, output = QTL.pvs.s,
                                  Q.names = QTL.names, mk.list = mk.list)
          
          QTL.pvs.adj.s <- update.res(input = R2.vs$part.adj.R2.sg/her,
                                      output = QTL.pvs.adj.s,
                                      Q.names = QTL.names, mk.list = mk.list)
          
          
          # difference QTL R2 values
          
          QTL.pts.d <- update.res(input = R2.ts$part.R2.diff/her, output = QTL.pts.d,
                                  Q.names = QTL.names, mk.list = mk.list)
          
          QTL.pts.adj.d <- update.res(input = R2.ts$part.adj.R2.diff/her,
                                      output = QTL.pts.adj.d,
                                      Q.names = QTL.names, mk.list = mk.list)
          
          QTL.pvs.d <- update.res(input = R2.vs$part.R2.diff/her, output = QTL.pvs.d,
                                  Q.names = QTL.names, mk.list = mk.list)
          
          QTL.pvs.adj.d <- update.res(input = R2.vs$part.adj.R2.diff/her,
                                      output = QTL.pvs.adj.d,
                                      Q.names = QTL.names, mk.list = mk.list)
          ...
          if(prob.prog){
            ...
            p.ts.adj[j, i] <- p.vs.adj[j, i] <- bias.adj[j, i] <- NA
  ...
  save.CV.res <- function(res, col, row, out, name){
    ...
    write.table(res, paste0(out, "/", name, ".txt"), quote = F, sep = "\t")
  ...
  save.CV.res(res = p.vs.adj, col = col.n, row = row.n, out = folder.loc,
              name = "p_vs_adj")
  
  save.CV.res(res = bias, col = col.n, row = row.n, out = folder.loc,
              name = "bias")
  
  save.CV.res(res = bias.adj, col = col.n, row = row.n, out = folder.loc,
              name = "bias_adj")
  ...
  av.pts.adj.s <- round(QTL.pts.adj.s/N.Qeff.est.ts, 1)
  av.pvs.adj.s <- round(QTL.pvs.adj.s/N.Qeff.est.vs, 1)
  bias.adj.s <- round(1 - (av.pvs.adj.s/av.pts.adj.s), 1)

  av.pts.d <- round(QTL.pts.d/N.Qeff.est.ts, 1)
  av.pvs.d <- round(QTL.pvs.d/N.Qeff.est.vs, 1)
  bias.d <- round(1 - (av.pvs.d/av.pts.d), 1)

  av.pts.adj.d <- round(QTL.pts.adj.d/N.Qeff.est.ts, 1)
  av.pvs.adj.d <- round(QTL.pvs.adj.d/N.Qeff.est.vs, 1)
  bias.adj.d <- round(1 - (av.pvs.adj.d/av.pts.adj.d), 1)

  QTL.sum <- data.frame(mppData$map, QTL.positions, av.pts.d, av.pvs.d,
                   bias.d, av.pts.adj.d, av.pvs.adj.d, bias.adj.d, av.pts.s,
                   av.pvs.s, bias.s, av.pts.adj.s, av.pvs.adj.s, bias.adj.s,
  ...
  write.table(QTL.sum, paste0(folder.loc, "/", "QTL.txt"), quote = F,
              sep = "\t", row.names = FALSE)
  ...
  write.table(profiles2, file, quote = F, sep = "\t", row.names = FALSE)
  ...
  return(list(QTL = QTL.sum, N.QTL = N.QTL, p.ts = p.ts, p.vs = p.vs,
              bias = bias, p.ts.adj = p.ts.adj, p.vs.adj = p.vs.adj,
              bias.adj = bias.adj, QTL.profiles = profiles2))

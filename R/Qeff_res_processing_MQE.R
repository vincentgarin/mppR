...
Qeff_res_processing_MQE <- function(Q.res, Q.eff, Q.pos, con.part, allele_ref,
  ...
  if (Q.eff == "cr"){
    ...
    index <- match(par.list, par.list.ref)
    Q.res <- Q.res[index, ]
    ...
    Q.res <- data.frame(Q.res, Con.part = unlist(ref.con),
    ...
    all.list <- unlist(mapply(FUN = function(x, ref) c(ref, x[-which(x == ref)]),
                              x = con.part, ref = allele_ref))

    all.list.ref <- substr(rownames(Q.res), nchar(as.integer(Q.nb)) + 2 ,
                           nchar(rownames(Q.res)))

    index <- match(all.list, all.list.ref)
    Q.res <- Q.res[index, ]
    ...
    all.ord <- substr(rownames(Q.res), nchar(as.integer(Q.nb)) + 2,
                      nchar(rownames(Q.res)))
    A <- A[, all.ord]
